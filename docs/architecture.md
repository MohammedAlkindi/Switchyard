...
Persisted agent identity is treated as a safety boundary, not trusted as an
arbitrary filesystem location or branch. Every record must resolve to the
direct child `.fleet/worktrees/<agent>` (including through symlinks and Windows
junctions), its branch must be exactly `fleet/<agent>`, and the state key,
record name, directory name, and branch suffix must agree. An escaping or
lookalike path such as `.fleet/worktrees-old/alice`, or a redirected branch,
makes the state corrupted; mutating commands refuse before touching Git, and
`fleet doctor --fix` rebuilds state only from registered `fleet/*` worktrees
inside the managed directory. The same checks are applied to an undo record
before `reset --hard` runs.
